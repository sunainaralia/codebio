...
function WhatWeDoSection() {
  ...
  return (
    <div className="py-[38px]  md:my-20 lg:my-0 flex flex-col justify-center md:items-center md:text-center px-[30px] md:px-0">
      ...
      <div className="w-full grid grid-cols-2 lg:grid-cols-[repeat(auto-fit,minmax(250px,1fr))] lg:gap-8 py-12 md:py-6  md:mt-28 lg:mt-20">
        {solutions.map((solution, index) => (
          <SolutionCard key={index} cardStyling={solution.cardStyling || ''} title={solution.title} icon={solution.icon} />
        ))}
      </div>

      <p className="text-[#2B2B2B] text-lg md:text-[30px] !leading-10 font-normal hidden md:block mt-[75px] lg:mt-[60px]">
