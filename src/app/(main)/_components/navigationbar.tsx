  ...
  <a href={link} target="_blank" rel="noopener noreferrer">
    <button onClick={onClick}>
      <Image
        ...
        height={60}
        width={60}
        className="w-[30px] h-[30px] lg:w-[35px] 4xl:w-[60px] lg:h-[35px] 4xl:h-[60px]"
      />
    </button>
  </a>
);
...
const MenueItemContainer = ({
  ...
  return (
    <div
      className={cn(
        "fixed z-[50] bottom-12 left-1/2 translate-x-[-50%] max-w-[1200px] w-[100vw] lg:w-max p-2 duration-300 transition-all",
    ...
    >
      <div
        className={`lg:hidden block ${
          showMenu ? "rounded-[30px]" : "rounded-[999px]"
        } w-full bg-radial-nav`}
      >
        <div
          className={`menu  ${
            showMenu ? "max-h-[400px] py-2 " : "max-h-0"
          ...
          {showMenu &&
            menuItems.map((item) =>
              item.title === "AI Labs" ? (
                ...
                <div
                  key={item.link}
                  className={`text-primary-white px-12 mx-2 py-5 ${
                    activeRoute !== "/" && item?.link?.includes(activeRoute)
        ...
        <div className="p-2 flex justify-between items-center gap-3 ">
          ...
          {!showMenu && (
            <div className="lg:hidden flex justify-between items-center text-primary-white gap-2">
              <Link
                href={activeRoute}
      ...
      <div className="bg-radial-nav p-2  hidden lg:flex justify-between items-center gap-3 rounded-[999px] w-full">
        <Link
          href={"/"}
          className="bg-[#dfdfdf85]  text-primary-white font-bold text-[20px] md:text-xl rounded-[999px] px-[17px] py-[14px] md:p-3 md:px-7 cursor-pointer"
        >
