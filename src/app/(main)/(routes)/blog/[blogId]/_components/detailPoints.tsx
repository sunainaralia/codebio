...
const DetailPoints = ({ data, indexvalue }: any) => {
    ...
    return (
        <div className="pb-[20px] xl:pb-20 4xl:pb-[125px]">
            {indexvalue !== 2 && indexvalue! !== 3 ? <>

                {
                    data.map((point: any, index: number) => (
                        <div className="pt-10 xl:pt-14 4xl:pt-[70px]" key={index}>
                            {/* text-xl sm:text-2xl md:text-3xl xl:text-[40px] 3xl:text-[52px] */}
                            {point.maintitle && <div className="text-xl sm:text-2xl xl:text-[28px] 2xl:text-4xl 4xl:text-[52px] leading-[27px] xl:leading-[3rem] 4xl:leading-[70px] font-bold ">
                                {point.maintitle}
                            </div>}
                            {/* text-lg sm:text-2xl md:text-3xl 2xl:text-[36px] 3xl:text-[42px] */}
                            <div className=" text-lg sm:text-xl xl:text-2xl 4xl:text-[42px] 4xl:leading-[56px] font-bold mt-5">
                                {point.title}
                            </div>
